...
mod tests {
    use super::{
        latex_runtime_compile_finish, latex_runtime_compile_start, CompileResult,
        LatexCompileFinishParams, LatexCompileStartParams, LatexError, LatexQueueStateInput,
    };
    use serde_json::Value;

    #[tokio::test]
    async fn start_queues_when_target_already_running() {
        let result = latex_runtime_compile_start(LatexCompileStartParams {
            tex_path: "/workspace/chapter.tex".to_string(),
            ...
            queue_state: Some(LatexQueueStateInput {
                ...
                build_extra_args: String::new(),
                pending_count: 1,
                source_path: "/workspace/main.tex".to_string(),
                reason: "save".to_string(),
        ...
        assert_eq!(
            result.get("shouldRun").and_then(Value::as_bool),
            Some(false)
        );
        assert_eq!(
            result
                .get("queueState")
                .and_then(|value| value.get("pendingCount"))
    ...
    #[tokio::test]
    async fn finish_requests_single_rerun_when_pending_work_exists() {
        let result = latex_runtime_compile_finish(LatexCompileFinishParams {
            tex_path: "/workspace/chapter.tex".to_string(),
            target_path: "/workspace/main.tex".to_string(),
            project_root_path: "/workspace/main.tex".to_string(),
            ...
            queue_state: Some(LatexQueueStateInput {
                ...
                build_extra_args: String::new(),
                pending_count: 3,
                source_path: "/workspace/chapter.tex".to_string(),
                reason: "save".to_string(),
                phase: "running".to_string(),
                updated_at: 0,
                scheduled_at: 0,
            ...
            result: CompileResult {
                ...
                log: "ok".to_string(),
                duration_ms: 1200,
                compiler_backend: Some("latexmk".to_string()),
                command_preview: None,
                requested_program: None,
                requested_program_applied: false,
            },
        ...
        assert_eq!(
            result.get("rerunRequested").and_then(Value::as_bool),
            Some(true)
        );
        assert_eq!(
            result
        ...
        assert_eq!(
            result.get("nextSourcePath").and_then(Value::as_str),
            Some("/workspace/chapter.tex")
        );
    }
}
