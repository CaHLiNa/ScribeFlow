...
mod tests {
    ...
    #[tokio::test]
    async fn session_start_primes_a_running_agent_session() {
        let started = ai_agent_session_start(AiAgentSessionStartParams {
            session: base_session(),
            skill: json!({ "id": "workspace-agent", "kind": "built-in-action" }),
            provider_state: json!({ "currentProviderLabel": "OpenAI", "model": "gpt-5.4" }),
            context_bundle: json!({ "workspace": { "available": true, "path": "/workspace" } }),
            user_instruction: "Inspect src/app.ts".to_string(),
            prompt_draft: "Inspect src/app.ts".to_string(),
            effective_permission_mode: "plan".to_string(),
            pending_assistant_id: "message:assistant".to_string(),
        ...
        assert_eq!(started.session["isRunning"].as_bool(), Some(true));
        assert_eq!(started.session["permissionMode"].as_str(), Some("plan"));
        assert_eq!(started.session["messages"].as_array().unwrap().len(), 2);
        assert_eq!(
            started.session["messages"][0]["content"].as_str(),
            Some("Inspect src/app.ts")
        );
    }

    #[tokio::test]
    async fn session_apply_event_tracks_runtime_requests_tasks_and_streaming() {
        let started = ai_agent_session_start(AiAgentSessionStartParams {
            session: base_session(),
            skill: json!({ "id": "workspace-agent", "kind": "built-in-action" }),
            provider_state: json!({ "currentProviderLabel": "OpenAI", "model": "gpt-5.4" }),
            ...
            user_instruction: "Inspect src/app.ts".to_string(),
            prompt_draft: "Inspect src/app.ts".to_string(),
            effective_permission_mode: "accept-edits".to_string(),
            pending_assistant_id: "message:assistant".to_string(),
            user_message_id: "message:user".to_string(),
            created_at: 1,
            fallback_title: "Run 1".to_string(),
        ...
        let permission = ai_agent_session_apply_event(AiAgentSessionApplyEventParams {
        ...
        .await
        .unwrap();
        let task = ai_agent_session_apply_event(AiAgentSessionApplyEventParams {
        ...
        .await
        .unwrap();
        let plan = ai_agent_session_apply_event(AiAgentSessionApplyEventParams {
        ...
        .await
        .unwrap();
        let text = ai_agent_session_apply_event(AiAgentSessionApplyEventParams {
        ...
        assert_eq!(
            text.session["permissionRequests"].as_array().unwrap().len(),
            1
        );
        assert_eq!(text.session["backgroundTasks"].as_array().unwrap().len(), 1);
        assert_eq!(
            text.session["backgroundTasks"][0]["taskId"].as_str(),
            Some("task-1")
        );
        assert_eq!(text.session["planMode"]["active"].as_bool(), Some(true));
        assert_eq!(
            text.session["messages"][1]["content"].as_str(),
            Some("Working on it")
        );
        assert_eq!(
            text.session["runtimeTransport"].as_str(),
            Some("anthropic-sdk")
        );
    }

    #[tokio::test]
    async fn tool_events_merge_upserts_by_tool_id() {
        let started = ai_agent_tool_events_merge(AiAgentToolEventsMergeParams {
            events: vec![],
            event: json!({
                "eventType": "tool",
                "toolId": "runtime:read",
        ...
        .await
        .unwrap();
        let updated = ai_agent_tool_events_merge(AiAgentToolEventsMergeParams {
        ...
        assert_eq!(updated.events[0]["status"].as_str(), Some("done"));
    }
}
