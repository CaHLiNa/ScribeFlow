...
mod tests {
    ...
    #[test]
    fn parse_bibtex_text_parses_minimal_article() {
        let content = r#"
@article{an2026cbf,
...
"#;
        let parsed = parse_bibtex_text(content);
        assert_eq!(parsed.len(), 1);
        assert_eq!(trim_string(parsed[0].get("citationKey")), "an2026cbf");
        assert_eq!(trim_string(parsed[0].get("typeKey")), "journal-article");
    }

    #[test]
    fn parse_ris_text_parses_conference_entry() {
        let content = r#"
TY  - CONF
...
ER  -
"#;
        let parsed = parse_ris_text(content);
        assert_eq!(parsed.len(), 1);
        assert_eq!(trim_string(parsed[0].get("typeKey")), "conference-paper");
        assert_eq!(trim_string(parsed[0].get("source")), "CDC");
    }
}
