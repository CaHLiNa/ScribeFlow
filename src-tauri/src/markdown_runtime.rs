...
mod tests {
    ...
    #[test]
    fn extracts_headings_and_bibliography_markers() {
        let items = extract_markdown_headings("# Intro\n\n## References\n").unwrap();

        assert_eq!(items.len(), 2);
        assert_eq!(items[0].kind, "heading");
        assert_eq!(items[0].text, "Intro");
        assert_eq!(items[0].level, 1);
        assert_eq!(items[1].kind, "bibliography");
        assert_eq!(items[1].text, "References");
        assert_eq!(items[1].level, 2);
    }

    #[test]
    fn preserves_text_from_inline_formatting_inside_headings() {
        let items = extract_markdown_headings("## A *bold* `code` $x+y$\n").unwrap();

        assert_eq!(items.len(), 1);
        assert_eq!(items[0].text, "A bold code x+y");
        assert_eq!(items[0].level, 2);
    }

    #[test]
    fn tracks_offset_and_line_for_multiline_documents() {
        let items = extract_markdown_headings("Intro\n\n### Deep section\n").unwrap();

        assert_eq!(items.len(), 1);
        assert_eq!(items[0].text, "Deep section");
        assert_eq!(items[0].level, 3);
        assert_eq!(items[0].line, Some(3));
        assert_eq!(items[0].offset, 7);
    }
}
