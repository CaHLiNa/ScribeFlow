...
mod tests {
    use super::{
        latex_compile_targets_resolve, latex_project_graph_resolve, normalize_fs_path,
        LatexAffectedRootsParams, LatexProjectGraphParams,
    };
    ...
    fn temp_dir_path(label: &str) -> std::path::PathBuf {
        std::env::temp_dir().join(format!("altals-latex-graph-{label}-{}", Uuid::new_v4()))
    }

    #[tokio::test]
    async fn resolves_root_and_outline_for_simple_project() {
        let root = temp_dir_path("root");
        ...
        .unwrap();
        fs::write(&bib, "@article{smith2024,\n title={A}\n}\n").unwrap();

        let result = latex_project_graph_resolve(LatexProjectGraphParams {
            source_path: normalize_fs_path(&chapter.to_string_lossy()),
            flat_files: vec![
                normalize_fs_path(&main.to_string_lossy()),
                normalize_fs_path(&chapter.to_string_lossy()),
                normalize_fs_path(&bib.to_string_lossy()),
            ],
            content_overrides: Default::default(),
        })
        .await
        .unwrap();

        let expected_root = normalize_fs_path(&main.to_string_lossy());
        assert_eq!(
            result.get("rootPath").and_then(Value::as_str),
    ...
    #[tokio::test]
    async fn compile_targets_resolve_root_for_tex_change() {
        let root = temp_dir_path("compile-target-tex");
        let chapter_dir = root.join("chapters");
        fs::create_dir_all(&chapter_dir).unwrap();
        let main = root.join("main.tex");
        let chapter = chapter_dir.join("chapter1.tex");
        ...
        .unwrap();
        fs::write(&chapter, "\\section{Intro}\n").unwrap();

        let result = latex_compile_targets_resolve(LatexAffectedRootsParams {
            changed_path: normalize_fs_path(&chapter.to_string_lossy()),
            flat_files: vec![
                normalize_fs_path(&main.to_string_lossy()),
                normalize_fs_path(&chapter.to_string_lossy()),
            ],
        ...
        assert_eq!(
            items[0].get("rootPath").and_then(Value::as_str),
            Some(expected_root.as_str())
        );
    ...
    #[tokio::test]
    async fn compile_targets_resolve_affected_roots_for_bib_change() {
        let root = temp_dir_path("compile-target-bib");
        let chapter_dir = root.join("chapters");
        fs::create_dir_all(&chapter_dir).unwrap();
        let main = root.join("main.tex");
        let chapter = chapter_dir.join("chapter1.tex");
        ...
        let result = latex_compile_targets_resolve(LatexAffectedRootsParams {
            changed_path: normalize_fs_path(&bib.to_string_lossy()),
            flat_files: vec![
                normalize_fs_path(&main.to_string_lossy()),
                normalize_fs_path(&chapter.to_string_lossy()),
                normalize_fs_path(&bib.to_string_lossy()),
