...
mod tests {
    ...
    #[test]
    fn queue_submission_moves_prompt_and_attachments() {
        let session = mutate_session(
            &json!({
                "promptDraft": "hello",
                "queuedPromptDraft": "",
                "attachments": [{ "id": "a1", "path": "/tmp/a.txt" }],
                "queuedAttachments": [],
            }),
            "queueSubmission",
            &Value::Null,
        );
        assert_eq!(session["promptDraft"].as_str(), Some(""));
        assert_eq!(session["queuedPromptDraft"].as_str(), Some("hello"));
        assert_eq!(session["attachments"].as_array().map(|v| v.len()), Some(0));
        assert_eq!(
            session["queuedAttachments"].as_array().map(|v| v.len()),
            Some(1)
        );
    }

    #[test]
    fn upsert_background_task_merges_existing_entry() {
        let session = mutate_session(
            &json!({
                "backgroundTasks": [{
                    "id": "task:1",
        ...
        assert_eq!(tasks[0]["label"].as_str(), Some("new"));
