...
mod tests {
    ...
    #[test]
    fn start_turn_marks_thread_running_and_creates_user_item() {
        ...
        let response = start_turn(
            ...
            RuntimeTurnStartParams {
                thread_id: thread.id.clone(),
                user_text: "Inspect the workspace".to_string(),
            },
        )
        .expect("turn start");

        assert_eq!(response.thread.status, ThreadStatus::Running);
        assert_eq!(response.turn.status, TurnStatus::Running);
        assert_eq!(response.items.len(), 1);
        assert_eq!(response.items[0].kind, ItemKind::UserMessage);
    }

    #[test]
    fn interrupt_turn_clears_active_turn() {
        let mut state = CodexRuntimeState::default();
        let thread = start_thread(
            &mut state,
            RuntimeThreadStartParams {
        ...
        let interrupted =
            interrupt_turn(&mut state, &thread.id, &started.turn.id).expect("turn interrupt");
        assert_eq!(interrupted.thread.status, ThreadStatus::Interrupted);
        assert_eq!(interrupted.turn.status, TurnStatus::Interrupted);
        assert!(interrupted.thread.active_turn_id.is_none());
    }
}
