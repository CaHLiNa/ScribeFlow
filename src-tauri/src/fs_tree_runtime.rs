...
mod tests {
    use super::{
        collect_loaded_directory_paths, list_ancestor_dir_paths, merge_preserving_loaded_children,
        patch_tree_entry_children,
    };
    use crate::fs_tree::FileEntry;
    ...
    fn dir(path: &str, children: Option<Vec<FileEntry>>) -> FileEntry {
        FileEntry {
            name: path.rsplit('/').next().unwrap_or_default().to_string(),
            path: path.to_string(),
            is_dir: true,
    ...
    #[test]
    fn merge_keeps_loaded_children() {
        let merged = merge_preserving_loaded_children(
            &[dir("/ws/docs", None)],
            &[dir("/ws/docs", Some(vec![file("/ws/docs/a.md")]))],
        );
        assert_eq!(
            merged[0].children.as_ref().map(|items| items.len()),
            Some(1)
        );
    }
    ...
    fn collect_loaded_dirs_walks_nested_tree() {
        ...
        assert_eq!(
            paths,
            vec!["/ws/docs".to_string(), "/ws/docs/chapters".to_string()]
        );
    }

    #[test]
    fn patch_children_updates_nested_directory() {
        let entries = vec![dir("/ws/docs", Some(vec![file("/ws/docs/a.md")]))];
        let patched = patch_tree_entry_children(&entries, "/ws/docs", &[file("/ws/docs/b.md")]);
        assert_eq!(
            patched[0]
                .children
                .as_ref()
    ...
    #[test]
    fn ancestor_dirs_list_parent_chain() {
        let ancestors = list_ancestor_dir_paths("/ws", "/ws/docs/chapters/ch1.md");
        assert_eq!(
            ancestors,
            vec!["/ws/docs".to_string(), "/ws/docs/chapters".to_string()]
        );
