...
use crate::references_query::document_reference_values_for_tex;
...
async fn references_write_bib_file_scoped(
    ...
    let references = if params.document_reference_selections.is_object() {
        document_reference_values_for_tex(
    ...
    let body = export_bibtex_content(&references);
...
mod tests {
    ...
    fn reference_runtime_file_params_normalize_raw_payloads() {
        ...
        assert!(bib_params.document_reference_selections.is_null());
        assert_eq!(bib_params.citation_style, "");

        let bib_params = reference_bib_file_params_from_payload(json!({
            "texPath": "/tmp/main.tex",
            "references": [{ "id": "ref-a" }],
            "documentReferenceSelections": { "/tmp/main.tex": ["ref-a"] },
            "citationStyle": "ieee"
        }));
        assert_eq!(bib_params.tex_path, "/tmp/main.tex");
        assert_eq!(bib_params.references.len(), 1);
        assert_eq!(
            bib_params.document_reference_selections["/tmp/main.tex"],
            json!(["ref-a"])
        );
    ...
    async fn scoped_reference_bib_write_stays_with_tex_workspace() {
        ...
        let bib_path = references_write_bib_file_scoped(
            ReferenceBibFileParams {
                ...
                document_reference_selections: Value::Null,
                references: vec![json!({
                    "id": "ref-1",
                    "citationKey": "lovelace2024",
    ...
    #[tokio::test]
    async fn reference_bib_write_resolves_document_references_in_rust() {
        let workspace_dir = std::env::temp_dir().join(format!(
            "scribeflow-reference-bib-document-target-{}",
            uuid::Uuid::new_v4()
        ));
        fs::create_dir_all(&workspace_dir).expect("create workspace dir");
        let tex_path = workspace_dir.join("main.tex");
        ...
        selections.insert(
            tex_path.to_string_lossy().trim().to_string(),
            json!(["ref-2", "missing", "ref-2"]),
        );

        let bib_path = references_write_bib_file_scoped(
            ReferenceBibFileParams {
                tex_path: format!(" {} ", tex_path.to_string_lossy()),
                citation_style: "apa".to_string(),
                document_reference_selections: Value::Object(selections),
                references: vec![
                    json!({
                        "id": "ref-1",
                        "citationKey": "lovelace2024",
        ...
        .await
        .expect("write selected document bib");

        let content = fs::read_to_string(&bib_path).expect("read generated bib");
        assert!(content.contains("hopper2025"));
        assert!(!content.contains("lovelace2024"));
        assert!(!content.contains("unused2026"));
