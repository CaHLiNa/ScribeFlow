...
mod tests {
    use super::{
        find_duplicate_reference_internal, merge_imported_references_internal, parse_csl_metadata,
    };
    use serde_json::json;
    ...
    fn duplicate_detection_matches_by_citation_key() {
        ...
        assert_eq!(
            duplicate.get("id").and_then(|value| value.as_str()),
            Some("a")
        );
    }
    ...
    fn csl_metadata_parser_extracts_title_and_category() {
        ...
        assert_eq!(
            style.get("name").and_then(|value| value.as_str()),
            Some("My Style")
        );
        assert_eq!(
            style.get("id").and_then(|value| value.as_str()),
            Some("my-style")
        );
        assert_eq!(
            style.get("category").and_then(|value| value.as_str()),
            Some("Numeric")
        );
    }
}
