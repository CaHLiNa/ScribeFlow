...
mod tests {
    ...
    #[test]
    fn csl_to_reference_record_maps_core_fields() {
        let reference = csl_to_reference_record(
            &json!({
                "id": "https://api.zotero.org/users/1/items/ABC123",
                "_key": "ABC123",
        ...
        assert_eq!(trim_string(reference.get("id")), "ABC123");
        assert_eq!(trim_string(reference.get("typeKey")), "journal-article");
        assert_eq!(trim_string(reference.get("citationKey")), "ABC123");
        assert_eq!(trim_string(reference.get("_zoteroLibrary")), "user/1");
    }

    #[test]
    fn same_reference_identity_matches_title_and_year_duplicates() {
        let current = json!({
        ...
        assert!(same_reference_identity(&current, &candidate));
    }
}
