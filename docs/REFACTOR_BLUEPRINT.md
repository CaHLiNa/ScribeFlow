...
The shell center and panel hosts no longer pay the full remount cost on every surface/panel switch either. The main `workspace` / `library` / `ai` center surfaces now swap through a cached shell view instead of a raw `v-if` chain, and the left/right sidebar hosts now cache their active `files` / `references` / `library` / `ai` and `outline` / `backlinks` / `details` surfaces across panel switches. In practice that means common navigation changes no longer rebuild those workbench surfaces from scratch every time the user toggles back and forth.
...
- A narrow 2026-03-24 performance slice has now reduced avoidable UI churn without widening the refactor scope: repeated clicks inside the already-active editor pane no longer reschedule editor-state persistence, document PDF routing no longer force-probes source kind on every preview-binding change, `PdfViewer.vue` now syncs toolbar/sidebar state from viewer events instead of a fixed 250 ms polling loop, and file-tree watch activity no longer treats every global key/mouse event as a signal to keep high-frequency polling alive.
- That same 2026-03-24 performance sequence now also keeps the main shell workbench surfaces and sidebar panel surfaces warm across common navigation switches instead of remounting them through raw conditional branches every time the user flips between project/library/AI or files/references/outline/backlinks.
