...
- `Phase 6` 暂未执行：当前仍缺少与 PDF.js 路线等价的文本选择/复制、搜索 UI 与若干桌面交互，直接切默认会导致回归
...
后续在切换默认 backend 前仍需补：

- 使用真实 `.pdf` 路径做手工验证
- 使用真实 `.tex` + `.synctex.gz` 路径做 `forward / reverse SyncTeX` 手工验证
- 验证默认 `pdfjs` backend 没有退化
- 验证 EmbedPDF 路线下的文本选择、搜索和桌面交互是否达到可替代标准
