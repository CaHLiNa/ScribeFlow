...
当前桌面端 PDF 预览最初建立在仓库内 vendored 的 PDF.js viewer 之上，并在外层叠加了大量 ScribeFlow 定制能力：
...
本计划文档现仅保留迁移背景与实施记录；`pdfjs` 删除工作已在后续清理切片中实际执行。

## 约束

- 默认用户路径必须继续走稳定 backend，避免打断当前桌面研究工作流。
...
- `src/components/editor/PdfIframeSurface.vue`（已删除）
...
- 组件曾直接依赖 PDF.js viewer internals
- 预览 backend 曾不可替换
...
曾新增 `src/services/pdf/pdfViewerBackend.js`，负责：
...
- `Phase 6` 已完成：默认 backend 已切到 `embedpdf`，随后用户批准删除 `pdfjs` 回滚 backend 与 vendored viewer
...
12. 删除 `pdfjs` backend selector、`PdfIframeSurface.vue`、`viewerUrl.js` 与 `public/pdfjs-viewer/*`

## 风险

### 高风险

- EmbedPDF 的页坐标能力是否足够支撑 reverse SyncTeX 仍需继续收敛
