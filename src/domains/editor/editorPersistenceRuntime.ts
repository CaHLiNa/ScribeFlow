...
export function createEditorPersistenceRuntime({
  ...
  return {
    async loadRecentFilesForWorkspace(workspaceDataDir, workspacePath) {
      return loadRecentFiles(workspaceDataDir, workspacePath)
    },

    async persistRecentFilesForWorkspace(workspaceDataDir, workspacePath, recentFiles = []) {
    ...
    async loadEditorStateSnapshot(workspaceDataDir) {
