...
export function classify(path) {
  return {
    isNewTab: isNewTab(path),
    isDraftPath: isDraftPath(path),
    isPreviewPath: isPreviewPath(path),
    isMarkdown: isMarkdown(path),
    isLatex: isLatex(path),
    isLatexEditorFile: isLatexEditorFile(path),
    isBibFile: isBibFile(path),
    isImage: isImage(path),
    isHtml: isHtml(path),
    isPdf: isPdf(path),
    isBinaryFile: isBinaryFile(path),
    previewSourcePath: previewSourcePathFromPath(path),
    viewerType: getViewerType(path),
    iconName: getFileIconName(basenamePath(path)),
