...
- reference exact-id lookup and presence checks are still partly domain-derived: selected-reference fallback, import result mapping, PDF asset actions, metadata refresh, document-reference insertion, and JSON export reuse `referenceStoreState.js` id helpers instead of duplicating array lookup rules in the Pinia store, while citation formatting target lookup has moved to Rust
