...
- reference import/add mutation outcome is now Rust-returned: `references_mutation_apply` returns imported count, reused-existing state, selected-reference id, selected reference payload and preferred selection for import/add flows, while `src/stores/references.js` commits the returned snapshot and consumes `mutation.result` directly without `referenceStoreState.js` result reconstruction
- reference import input state is still JS-derived for UI empty-state gating: empty import result shape and import list validation remain in `referenceStoreState.js` until Rust exposes an import-intent/preflight contract
