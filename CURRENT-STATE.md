...
- reference import result state is now domain-derived too: empty import result shape, import input list validation, selected-reference lookup, imported-count coercion, and reused-existing result flags live in `referenceStoreState.js`, while `src/stores/references.js` only runs import services, dispatches the Rust merge mutation, commits the returned snapshot, and manages `importInFlight`
- reference import mutation commit state is now domain-derived too: selected-reference id extraction for imported-reference snapshot commits lives in `referenceStoreState.js`, while `src/stores/references.js` only dispatches the Rust merge mutation and commits the returned snapshot with the derived selection hint
- reference add mutation result state is now domain-derived too: selected-reference id extraction and post-commit selected-reference lookup live in `referenceStoreState.js`, while `src/stores/references.js` keeps add mutation dispatch and snapshot commit side effects
- reference metadata refresh target state is now domain-derived too: refresh target id trimming and current-reference lookup live in `referenceStoreState.js`, while `src/stores/references.js` keeps backend metadata refresh, update mutation dispatch, and snapshot commit side effects
- reference PDF asset target/result state is now domain-derived too: attach/rename target id trimming, target-reference lookup, and post-update current-reference fallback live in `referenceStoreState.js`, while `src/stores/references.js` keeps asset storage/rename, update mutation dispatch, and snapshot commit side effects
- reference removal target/result state is now domain-derived too: remove target id trimming, target-reference lookup, and removed-result gating live in `referenceStoreState.js`, while `src/stores/references.js` keeps Rust mutation dispatch, snapshot commit, and Zotero delete side effects
- reference update mutation result state is now domain-derived too: changed-result gating for update mutations lives in `referenceStoreState.js`, while `src/stores/references.js` keeps update mutation dispatch, commit selection, and snapshot commit side effects
- reference PDF import target state is now domain-derived too: selected-reference id trimming, imported snapshot fallback, target-reference lookup, and can-import gating live in `referenceStoreState.js`, while `src/stores/references.js` keeps PDF metadata import, asset storage, Rust mutation dispatch, and snapshot commit side effects
- reference PDF import result state is now domain-derived too: final selected-reference lookup after the imported PDF asset commit lives in `referenceStoreState.js`, while `src/stores/references.js` only returns the derived result after Rust mutation and asset persistence side effects finish
- reference collection mutation result state is now domain-derived too: changed gating and collection-object fallback for create/rename collection mutations live in `referenceStoreState.js`, while `src/stores/references.js` keeps Rust mutation dispatch and snapshot commit side effects
- reference mutation boolean result state is now domain-derived too: remove-collection removed checks, document-reference-id changed checks, and reference collection toggle changed/toggled-on flags live in `referenceStoreState.js`, while `src/stores/references.js` keeps mutation dispatch, snapshot commit, and Zotero/delete side effects
...
- reference Zotero sync result state is now domain-derived too: skipped-state handling, count coercion, selected-reference id fallback, snapshot fallback, and sync status/last-sync result fields live in `referenceStoreState.js`, while `src/stores/references.js` keeps backend sync calls, snapshot application, timestamp generation, and error classification in the store/service boundary
- reference citation formatting target state is now domain-derived too: citation-format target lookup and can-format gating live in `referenceStoreState.js`, while `src/stores/references.js` keeps the formatter service call and workspace-path side effect
...
- reference selected-row fallback is now domain-derived too: selected-reference lookup and filtered-list first-row fallback live in `referenceStoreState.js`, while `src/stores/references.js` keeps only the public getter surface
...
- reference JSON export target state is now domain-derived too: JSON export target lookup and can-export gating live in `referenceStoreState.js`, while `src/stores/references.js` keeps the missing-reference user error and JSON file-writing service call
- reference exact-id lookup and presence checks are now domain-derived too: selected-reference fallback, import result mapping, PDF asset actions, metadata refresh, document-reference insertion, citation formatting, and JSON export all reuse `referenceStoreState.js` id helpers instead of duplicating array lookup rules in the Pinia store
- reference section and sort selection guards are now domain-derived too: section key fallback and reference sort-key normalization live in `referenceStoreState.js`, while `src/stores/references.js` only coordinates state updates and query refresh
- reference resolved-query hydration is now domain-derived too: Rust query fallback, query field extraction, selected-reference fallback priority, and sort-key normalization live in `referenceStoreState.js`, while `src/stores/references.js` only invokes the runtime query and writes the derived state back
- reference sidebar/snapshot selection reconciliation is now domain-derived too: sidebar section/source/collection/tag reset rules, sort selection fallback, row selection validation, and snapshot selection pruning live in `referenceStoreState.js`, while `src/stores/references.js` only applies helper results before query refresh or snapshot sync
- reference document-reference mutation derivation is now domain-derived too: TeX path normalization, non-array id fallback, add/remove duplicate guards, and next-id-list calculation live in `referenceStoreState.js`, while `src/stores/references.js` only dispatches the resulting `setDocumentReferenceIds` mutation through Rust
...
- reference store lifecycle shape is now domain-derived too: initial state, cleanup/reset state, and persisted library snapshot payload assembly live in `referenceStoreState.js`, while `src/stores/references.js` keeps persistence, Rust mutation dispatch, loading lifecycle, and workspace side effects
- reference snapshot apply state is now domain-derived too: snapshot defaulting, collection/tag/reference array fallbacks, document-reference selection shape fallback, citation-style fallback, selection reconciliation, and PDF dock snapshot reconciliation are produced by `buildReferenceSnapshotApplyState`, while `src/stores/references.js` only writes the derived fields, refreshes the query runtime, and performs the workspace details fallback side effect
- reference mutation commit selection is now domain-derived too: update/remove commit preferred-selection fallback and removed-reference selection clearing live in `referenceStoreState.js`, while `src/stores/references.js` only commits the Rust-returned mutation snapshot with the derived selection hint
