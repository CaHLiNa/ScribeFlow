...
- reference removal target state is still domain-derived for UI/Zotero side-effect gating, but removed-result gating and preferred-selection clearing now come from the Rust mutation outcome
...
- reference collection/document-reference/toggle mutation result state now comes directly from Rust: create/rename collection, remove collection, set document reference ids, and toggle collection actions consume `mutation.result` without JS result wrappers
