...
- extension task presentation is now shared too: right-sidebar task rows derive title, status tone, row tone, group counts, group tone, progress copy, progressbar metadata, target facts, result counts, artifact counts, preview/action result groups, and quick action affordances from one pure presentation helper instead of duplicating running/recent UI branches
