...
- extension task presentation is now shared too: right-sidebar task rows derive title, status tone, progress copy, progressbar metadata, target facts, result counts, artifact counts, and preview/action result groups from one pure presentation helper instead of duplicating running/recent UI branches
