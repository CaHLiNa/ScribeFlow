...
- reference exact-id lookup for synchronous reference/query surfaces now comes from Rust query DTOs: `referenceResolvedQueryDto.js` only reads Rust-returned lookup maps for existing synchronous store APIs; the extra `hasReferenceById` and `resolveSelectedReference` DTO helpers are gone, selected reference reads the Rust-returned `selectedReference` field directly, and PDF dock stale-tab pruning uses the Rust-normalized snapshot already passed to the UI dock helper
