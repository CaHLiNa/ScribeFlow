...
- extension task timelines now expose visible/hidden recent counts as a domain contract: document-plugin task panels render the newest recent tasks with a compact older-task footer when history is truncated, while action panels and task rows consume the same workspace-scoped timeline instead of drifting across separate recent-task slices
