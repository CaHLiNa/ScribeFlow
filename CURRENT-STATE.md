...
- reference search filtering is now Rust-query-owned too: `references_query_search` receives the current reference snapshot, document-reference selections, TeX path, query and sort key, returns matching library/document/available reference DTOs plus library lookup maps, and CitationPalette/DocumentReferencesPanel consume those async results through `searchReferenceQuery` instead of rebuilding search haystacks or synchronously reading selected document-reference DTOs in JS
...
- reference lookup readers are now narrower: `referenceResolvedQueryDto.js` no longer exposes exact-id helpers (`hasReferenceById` or `resolveReferenceById`) and no longer wraps selected references; `selectReference()` stores raw selection intent and waits for Rust query normalization, selected reference reads Rust's returned `selectedReference` field directly, and the remaining DTO readers only adapt citation-key/document-reference lookup maps for synchronous editor/citation APIs
