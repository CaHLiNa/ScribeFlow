...
- reference exact-id lookup and presence checks are still partly domain-derived: selected-reference fallback and document-reference lookup/search reuse `referenceStoreState.js` id helpers instead of duplicating array lookup rules in the Pinia store, while citation formatting, export target lookup, metadata refresh target lookup, remove target lookup, document-reference mutation derivation, PDF asset target lookup and PDF import target/result lookup have moved to Rust
