...
- reference search filtering is now Rust-query-owned too: `references_query_search` receives the current reference snapshot, document-reference selections, TeX path, query and sort key, returns matching library/document/available reference DTOs plus library lookup maps, and CitationPalette/DocumentReferencesPanel consume those async results instead of rebuilding search haystacks or synchronously reading selected document-reference DTOs in JS
