...
- reference store bootstrap shell remains transitional: initial transient UI flags and persisted library snapshot payload assembly still live in `referenceStoreState.js`, but initial `resolvedQueryState` is `null` until Rust `references_query_resolve` returns the first query DTO
