...
- reference exact-id lookup and presence checks are still partly domain-derived: selected-reference fallback and document-reference insertion reuse `referenceStoreState.js` id helpers instead of duplicating array lookup rules in the Pinia store, while citation formatting, export target lookup, metadata refresh target lookup, PDF asset target lookup and PDF import target/result lookup have moved to Rust
