...
- reference store bootstrap shape remains transitional: initial state and persisted library snapshot payload assembly still live in `referenceStoreState.js`, while `src/stores/references.js` keeps persistence, Rust mutation dispatch, loading lifecycle, and workspace side effects
