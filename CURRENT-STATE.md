...
- reference exact-id lookup and presence checks are still partly domain-derived: selected-reference fallback, import result mapping, metadata refresh, and document-reference insertion reuse `referenceStoreState.js` id helpers instead of duplicating array lookup rules in the Pinia store, while citation formatting, export target lookup and PDF asset target lookup have moved to Rust
