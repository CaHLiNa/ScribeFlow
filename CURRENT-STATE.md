...
- reference resolved-query hydration is now Rust-backed too: `src/stores/references.js` no longer builds a pending/default query DTO in JS; `referenceStoreState.js` only accepts Rust-returned query DTOs and maps returned fields back to store selection state
- reference sidebar selection reconciliation no longer pre-validates in JS: `src/stores/references.js` clears mutually exclusive UI filters, sends the raw selected key through the query bridge, and hydrates the normalized query result; snapshot selection pruning remains a transitional `referenceStoreState.js` adapter
...
- reference cleanup/reset is now Rust-backed too: `referencesStore.cleanup()` clears transient UI flags, then applies an empty snapshot through `references_snapshot_normalize` and hydrates selection/filter state through `references_query_resolve`; workspace close awaits that cleanup before closing the workspace
- reference store bootstrap shell remains transitional: initial transient UI flags still live in `referenceStoreState.js`, but persisted library snapshot payload assembly now goes through Rust `references_snapshot_payload_build`, and initial `resolvedQueryState` is `null` until Rust `references_query_resolve` returns the first query DTO
- reference snapshot apply normalization is now Rust-owned too: `applyLibrarySnapshot()` sends raw snapshots through `references_snapshot_normalize`, then uses `references_query_resolve` to hydrate selection/filter state; JS only writes the returned snapshot fields and keeps PDF dock reconciliation as a UI helper
