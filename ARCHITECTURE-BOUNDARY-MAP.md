...
| `src/components/references/ReferenceLibraryWorkbench.vue` | 802 | Reference workbench orchestration; Phase 8 extracted toolbar/table presentation, deterministic workbench helpers, and context-menu presentation groups. |
...
- 2026-05-18: Extension task timeline density is now domain-backed too: `src/domains/extensions/extensionStoreState.js` computes full workspace-scoped running/recent totals plus visible/hidden recent counts, and `src/domains/extensions/extensionTaskPresentation.js` derives the older-task footer copy. `DocumentPluginsPanel.vue` and `ExtensionTaskPanel.vue` consume the same timeline contract, while `scripts/probe-extension-task-timeline.mjs` and `scripts/probe-extension-document-action-task-surface.mjs` guard truncation and footer rendering.
