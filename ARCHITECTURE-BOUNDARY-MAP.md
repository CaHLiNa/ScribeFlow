...
- 2026-05-18: Reference snapshot apply normalization moved back to Rust too. `applyLibrarySnapshot()` now sends raw snapshots through `references_snapshot_normalize`, then hydrates selected reference/filter state through `references_query_resolve`; `referenceStoreState.js` no longer owns snapshot apply or snapshot selection reconciliation helpers, and `src/services/references/referenceLibraryIO.js` no longer hardcodes the default snapshot shape. JS keeps PDF dock snapshot reconciliation as UI-only state.
- 2026-05-18: Reference cleanup/reset now reuses the Rust snapshot/query path. `referencesStore.cleanup()` clears transient UI flags and applies an empty snapshot through `references_snapshot_normalize` plus `references_query_resolve`, and workspace close awaits that async cleanup before closing the workspace. `buildReferenceStoreCleanupState` was removed from `referenceStoreState.js`; initial/default query adapters remain as the next cleanup target.
