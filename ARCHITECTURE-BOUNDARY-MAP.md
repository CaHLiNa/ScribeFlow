...
- 2026-05-18: Reference snapshot apply normalization moved back to Rust too. `applyLibrarySnapshot()` now sends raw snapshots through `references_store_state_build`, which combines snapshot normalization with `references_query_resolve`; `referenceStoreState.js` no longer owns snapshot apply or snapshot selection reconciliation helpers, and `src/services/references/referenceLibraryIO.js` no longer hardcodes the default snapshot shape. JS keeps PDF dock snapshot reconciliation as UI-only state.
- 2026-05-18: Reference cleanup/reset now reuses the Rust state-builder path. `referencesStore.cleanup()` clears transient UI flags and applies an empty snapshot through `references_store_state_build`, and workspace close awaits that async cleanup before closing the workspace. `buildReferenceStoreCleanupState` was removed from `referenceStoreState.js`; JS no longer builds default resolved-query DTOs.
- 2026-05-18: Reference default resolved-query adapters and residual key normalization helpers were removed from JS. `referencesStore` no longer builds pending/default query DTOs before Rust returns; initial `resolvedQueryState` is `null`, `applyLibrarySnapshot()` and explicit query refreshes hydrate through `references_query_resolve`, and `referenceStoreState.js` no longer exports collection/tag/sort/default-query normalization helpers.
- 2026-05-18: Reference persisted snapshot payload assembly moved back to Rust. `references_snapshot_payload_build` now receives the current store state and returns the canonical normalized snapshot payload, while `src/stores/references.js` calls it through the thin `referenceLibraryIO` bridge before mutation, import, PDF import, Zotero sync and direct persistence flows. `referenceStoreState.js` no longer exports `buildReferenceLibrarySnapshotPayload` or hardcodes persisted snapshot schema fields.
- 2026-05-18: Reference store bootstrap state assembly moved back to Rust. `references_store_state_build` now returns canonical library/source sections, normalized snapshot fields, selected filter/sort/reference keys and the resolved query DTO; `buildReferenceStoreInitialState()` only creates the synchronous Pinia UI shell, app startup hydrates canonical defaults through Rust, and the old `referenceLibraryFixtures.js` JS authority file was deleted.
...
- 2026-05-18: Reference query target lookup moved back to Rust. `references_query_resolve` now accepts `documentReferenceSelections` and returns selected reference/collection/tag targets, library lookup maps, Rust-built search indexes, and per-document selected/available reference DTOs. `src/stores/references.js` keeps the existing synchronous editor/citation API but reads from `resolvedQueryState`; `referenceResolvedQueryDto.js` adapts those Rust DTOs, while `referenceStoreState.js` is now limited to UI state/display helpers. `scripts/probe-reference-query-rust-normalization.mjs` and `scripts/probe-reference-store-state-contract.mjs` guard that JS does not scan library arrays, reconstruct search haystacks, or put query DTO readers back into `referenceStoreState.js`.
- 2026-05-19: Reference resolved-query DTO fallback state was removed from JS. `referenceResolvedQueryDto.js` now only accepts Rust-returned query DTO objects, selected references must come from the returned `selectedReference` field, and selection hydration no longer falls back to the previous Pinia `selectedReferenceId`. `scripts/probe-reference-store-state-contract.mjs` guards that prior-state and filtered-row fallbacks do not return.
- 2026-05-19: Reference search filtering moved back to Rust. `references_query_search` now returns matching library, document-selected, and document-available reference DTOs from the current snapshot/query/sort/document-selection inputs; `src/stores/references.js` exposes async search actions, and CitationPalette plus DocumentReferencesPanel consume those DTOs instead of rebuilding search haystacks or filtering over `referenceSearchIndex` in JS.
...
- 2026-05-19: DocumentReferencesPanel moved its selected-reference and missing-citation link lookup path to the async Rust search DTO. `references_query_search` now returns `referenceLookup`, `src/stores/references.js` preserves that lookup, and the sidebar panel no longer calls synchronous reference DTO readers for selected document references or missing citation keys.
- 2026-05-19: The unused `searchAvailableReferencesForDocument()` store wrapper was removed after DocumentReferencesPanel switched to direct `searchReferenceQuery` consumption. Available-reference filtering remains Rust-owned through `references_query_search`, and the store public surface no longer exposes the extra document-search convenience action.
