...
- 2026-05-18: Reference store export selection rules moved into `src/domains/references/referenceStoreState.js`. `exportBibTeXAsync()`, `writeBibTeXExportFile()`, and `writeReferenceJsonExportFile()` keep their public store API for reference workbench actions, but exact-id lookup and ordered export-list filtering are now pure domain helpers while export serialization and file writes stay in `src/services/references/bibtexExport.js`. `scripts/probe-reference-store-export-workflow-contract.mjs` guards that store export actions do not duplicate selection rules inline.
